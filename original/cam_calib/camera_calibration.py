...
############################ IMAGE PROCESS #####################################
    images = glob.glob("/workspace/python/arUco/cam_calib/calib/*.jpg") # route for chessboard photos
    for pic in images:
        img = cv.imread(pic)
        ...
        if ret:
            ...
            # cv.drawChessboardCorners(img,(w,h), corner2,ret) # show chessboard corners
            # cv.imshow('img',img)
            # cv.waitKey(0)
            # cv.destroyAllWindows()
...
def cam_calib_charuco():
        ...
        for images in glob.glob(".\calib\*.jpg"):
            ...
            corners, ids = aruco.detectMarkers(img, marker_dict)
            if len(ids) > 0:
                a, b = aruco.interpolateCornersCharuco(corners, ids, img,board,None, None)
                l_corners.append(a)
                l_ids.append(b)
...
def display_calib_rst(mtx, dist):
    '''
    show calibrated images
    '''
    images = glob.glob(".\cam_calib\calib\*.jpg") # route for chessboard photos
...
if __name__ == "__main__":
    rt, mtx, dist, rvecs, tvecs = cam_calib()
    # cam_calib_charuco()
    print(mtx)
    print(dist)
    display_calib_rst(mtx, dist)
